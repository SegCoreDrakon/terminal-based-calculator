...
void Calculator::operation() {
    debugsys::write_log("operation function called");
    UI.clear();
    UI.menu_display();
...
void Calculator::m_set_result() {
    debugsys::write_log("m_set_result function called");
...
void Calculator::m_input_number() {
    ...
    debugsys::write_log("number 1 set");

    UI.move(1, 2, true, false);

    double number2 {input()};
    debugsys::write_log("number 2 set");
...
double Calculator::input() {
    debugsys::write_log("input function called");

    double userinput {};
    std::cout << "number   | ";
    std::cin >> userinput;
    debugsys::write_log("user set a number");
    std::cin.clear();
    std::cin.ignore(9999, '\n');
...
void Calculator::m_handle_choice() {
    debugsys::write_log("m_handle_choice function called");
    ...
    do {
        ...
        switch (int(oprt)) {
            case 1:
                debugsys::write_log("addition option triggered");
                m_input_number();
                result(m_addition(m_number1, m_number2));
                break;

            case 2:
                debugsys::write_log("substraction option triggered");
                m_input_number();
                result(m_subtraction(m_number1, m_number2));
                break;

            case 3:
                debugsys::write_log("multiplication option triggered");
                m_input_number();
                result(m_multiplication(m_number1, m_number2));
                break;

            case 4:
                debugsys::write_log("division option triggered");
                m_input_number();
                result(m_division(m_number1, m_number2));
                break;

            case 5:
                debugsys::write_log("check logs option triggered");
                calc_sys.check_logs();
                break;

            case 6:
                debugsys::write_log("delete logs option triggered");
                calc_sys.delete_logs();
                operation();
                break;

            case 7:
                debugsys::write_log("leave option triggered");
                calc_sys.leave_program();
                break;

            default:
                debugsys::write_log("unknow option triggered");
                ...
                oprt = 0;
                if (int(oprt) == 0) {
                    debugsys::write_log("operator reseted");
                    UI.clear();
                    std::cin.clear();
                    UI.menu_display();
                    UI.move(2, 13, false, false);
...
std::string Calculator::m_operation_type(int oprt_type) {
    debugsys::write_log("m_operation called");
...
void Calculator::result(double computation) {
    debugsys::write_log("result function called");
